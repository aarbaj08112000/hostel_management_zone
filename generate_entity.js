...
const outputDir = path.join(__dirname, 'src/hostle/entity');
...
// Function to generate entity content
...
`;
  ...
  for (const [colName, colType] of Object.entries(columns)) {
    ...
    if (colName.endsWith('_id') || colName.endsWith('Id')) {
      if (colName === Object.keys(columns)[0]) {
        colDecorator = '@PrimaryGeneratedColumn()';
      } else {
        colDecorator = '@Column()';
      }
    } else if (colType === 'Date') {
      colDecorator = '@Column({ type: "datetime", nullable: true })';
    } else if (colType === 'boolean') {
      colDecorator = '@Column({ type: "boolean", default: false })';
    } else if (colType === 'number') {
      colDecorator = '@Column({ type: "decimal", nullable: true })';
    } else {
      colDecorator = '@Column({ type: "varchar", nullable: true })';
    }

    content += `  ${colDecorator}\n  ${colName}: ${tsType};\n\n`;
  }
...
for (const [tableName, columns] of Object.entries(tables)) {
  const filePath = path.join(outputDir, `${tableName}.entity.ts`);
  const entityContent = generateEntity(tableName, columns);

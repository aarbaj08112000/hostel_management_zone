...
export class BrandService {
  protected readonly log = new LoggerHandler(BrandService.name).getInstance();
  ...
  async startBrand(reqObject, reqParams) {
    let outputResponse = {};
    try {
      ...
      inputParams = await this.getBrand(inputParams);
      if (!_.isEmpty(inputParams.brand)) {
        outputResponse = this.brandFinishedSuccess(inputParams);
      } else {
        outputResponse = this.brandFinishedFailure(inputParams);
      }
    } catch (err) {
      this.log.error('API Error >> brand >>', err);
    }
    return outputResponse;
  }
  async getBrand(inputParams: any) {
    ...
    try {
      ...
      const data = await Promise.all(
        results.hits.map(async (hit) => {
          fileConfig.image_name = hit._source['brand_image'];
          fileConfig.path = `brand_${aws_folder}`;
          if (hit._source['brand_image']) {
            hit._source['brand_image'] = await this.general.getFile(fileConfig, inputParams);
          }
          return hit._source;
        })
      );
    ...
    inputParams.brand = this.blockResult.data;
    return inputParams;
  }
  brandFinishedSuccess(inputParams: any) {
    const settingFields = {
      status: 200,
      success: 1,
  ...
  brandFinishedFailure(inputParams: any) {
    const settingFields = {
      status: 200,
      success: 0,

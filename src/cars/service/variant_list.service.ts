...
export class VariantListService {
  ...
  async startVariantList(reqObject, reqParams) {
    let outputResponse = {};
    try {
      ...
      inputParams = await this.getVariantList(inputParams);
      if (!_.isEmpty(inputParams.variant_list)) {
        outputResponse = this.variantListFinishedSuccess(inputParams);
      } else {
        outputResponse = this.variantListFinishedFailure(inputParams);
      }
    } catch (err) {
      this.log.error('API Error >> variant_list >>', err);
    }
    return outputResponse;
  }
  async getVariantList(inputParams: any) {
    this.blockResult = {};
    try {
      let index = 'nest_local_variant_list';
    ...
    inputParams.variant_list = this.blockResult.data;
    return inputParams;
  }
  variantListFinishedSuccess(inputParams: any) {
    const settingFields = {
      status: 200,
      success: 1,
      message: custom.lang('variant list found.'),
      fields: [],
    };
    const outputKeys = ['variant_list'];
    settingFields.fields = ['variant_id', 'variant_name', 'variant_code', 'status', 'added_date', 'updated_by', 'added_by', 'updated_date', 'added_name', 'updated_name'];
    const outputData: any = {};
    outputData.settings = { ...settingFields, ...this.settingsParams };
    outputData.data = inputParams;

    const funcData: any = {};
    funcData.name = 'variant_list';

    funcData.output_keys = outputKeys;
    funcData.multiple_keys = this.multipleKeys;
    return this.response.outputResponse(outputData, funcData);
  }
  variantListFinishedFailure(inputParams: any) {
    const settingFields = {
      status: 200,
      success: 0,
      message: custom.lang('variant list not found.'),
      fields: [],
    };
